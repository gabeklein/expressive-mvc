...
Lot of closures being created here too, which can harm performance if the
...
With this our component officially dumb, which is a good thing! Now, it's entirely managed and only contains display logic.
...
## Migrating from `useCallback`

The often overlooked `useCallback` hook is a great way to avoid unnecessary renders caused by the unnecessary creation of new function-objects.
