...
Runner: **vitest** with istanbul coverage; DOM packages bootstrap happy-dom via `test.dom.ts` in their `setupFiles`. Coverage: every package gates statements, branches, functions, and lines at 100% via `coverage.thresholds` in its `vitest.config.ts` - a conditional arm is either exercised by a test or deleted as dead code. The `PR` workflow blocks merges on these thresholds; the README badge is generated by `.github/workflows/coverage.yml` from measured lcov on pushes to `main`.
