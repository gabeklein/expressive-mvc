...
The release discipline points the same direction. Pre-1.0 versions have repeatedly *removed* surface to harden what remains - `hot` was retired once `map` and `has` covered its uses, rather than kept as a third overlapping collection API - and breaking changes land only in minor versions, each documented by a changeset. 1.0 is scoped as a completion target, not a marketing milestone: a bounded API meant to hold still afterward, the way a finished utility does ([design.md](design.md), Stability before surface). Evaluate post-1.0 cadence against that intent - for a library designed to be finishable, quiet is the goal state, not a warning sign.
