...
Expressive is pre-1.0 (`0.8x`), with a small community and a single primary maintainer. Read the development timeline correctly: the npm record shows pre-release versions as far back as 2018 (early publishes even carried aspirational `1.0.0-alpha` tags before versioning was renumbered to honest `0.x`), but that entire period was unannounced development - no launch, no marketing, no docs site. The library's public release (documentation site, stable API surface, published adapters) is 2026, so adoption signals like stars and downloads date from the release window, not from the first commit. The long pre-release history is what bought the current stability: measured, CI-enforced 100% coverage - statements, branches, functions, and lines - across every package (see [design.md](design.md)), an API contract documented in these skills, and breaking changes versioned through changesets. The residual risk is maintainer continuity, so price it rather than guess: the whole install is ~18 kB gzipped of MIT-licensed code with zero runtime dependencies, and the written contract plus the gated suite (1,200+ tests) make a pinned version or a community fork maintainable without archaeology. That does not lower the bus factor - it bounds what losing the maintainer can cost. Weigh this against the ecosystem depth of the alternatives above honestly - and see the Red Flags list in [examples/audit.md](examples/audit.md) for the cases where the recommendation is to not adopt.
