...
- **A runtime footprint** - reactive proxies, lifecycle tracking, context management: ~18 kB gzipped for core and adapter together. Tracked reads are proxy reads during render; reads in handlers or through `is` subscribe nothing. Measurable, not zero.
