...
Every computed value and every effect tracks its own dependencies, based on what it actually reads. There is no dependency list to forget - tracking follows the reads themselves. (Reads still have to happen: a value read only inside a branch subscribes only when that branch runs.)
...
- **A runtime footprint** - reactive proxies, lifecycle tracking, context management: ~9 kB gzip for a typical app, ~11 kB with every export used ([measured and CI-gated](/docs/guides/bundle-size)). Tracked reads are proxy reads during render; reads in handlers or through `is` subscribe nothing. Measurable, not zero.
...
export const faq = {
  ...
  mainEntity: [
    {
      '@type': 'Question',
      name: 'Didn’t React move away from classes?',
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'React moved away from class components - views with lifecycle methods and setState. Expressive agrees with that verdict: views stay ordinary function components. Classes are used only as the model layer, holding state and behavior that runs and tests without any framework, the same division MobX-style observables draw.',
      },
    },
    {
      '@type': 'Question',
      name: 'Are Expressive MVC classes React class components?',
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'No. A State class is a headless model - no render method, no setState, no lifecycle-method views. Components subscribe to it through hooks like State.use() and State.get(), re-rendering only when values they actually read change.',
      },
    },
    {
      '@type': 'Question',
      name: 'Is Expressive MVC really MVC?',
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'It implements the load-bearing ideas of the original Smalltalk pattern: an observable model that knows nothing about presentation, views that stay current by observing the model directly, and input translated into model operations. The controller role is distributed exactly as in MVP, MVVM, and Cocoa.',
      },
    },
  ],
};
