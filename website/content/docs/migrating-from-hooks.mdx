...
> App-wide singletons don't even need the Provider - a module-scope `Theme.new()` registers into the root context, findable by `Theme.get()` from anywhere. [See it in the Playground.](/examples/composition/singletons)
...
Copy-on-write is the habit worth keeping from hooks. But when a collection is mutated *a lot* - a game board, a live buffer, rows edited in place - wrap it with `hot()` and mutate away:
...
class Game extends State {
  ...
  play(index: number, mark: string) {
    this.board[index] = mark; // updates, in place
  }
}
```

Hot values track per-key, so a component reading `board[4]` only re-renders when that cell changes. See [Arrays & Collections](/docs/guides/collections) for the full contract, or [poke the tic-tac-toe example](/examples/apps/tictactoe) to watch it work.
