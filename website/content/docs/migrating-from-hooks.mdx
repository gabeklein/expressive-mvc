...
### 3a. Separate source state from derived state
...
Writes change from `setName(x)` to `form.name = x`. Derived values should usually move to getters, not to extra fields that need to be synchronized.
