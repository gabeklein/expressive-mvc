...
class Viewport extends State {
  width = Infinity; // desktop-first until measured
...
`mount()` runs once when the owning component commits - and never during server render, which is why `window` is safe inside it where a field initializer or `new()` would crash on the server. See [lifecycle hooks](/docs/api/state#lifecycle-hooks).
...
function Header() {
  ...
  // resolves from Control.theme
}
```

> App-wide globals don't even need the Provider - declare the class [`static global`](/docs/guides/context#globals-and-the-root-context) and a module-scope `Theme.new()` registers into the root context, findable by `Theme.get()` from anywhere. [See it in the Playground.](/examples/composition/globals)
...
The bridge runs the other way too. If an instance already lives outside React - a module-scope controller, a state owned by a non-React system - the free `use()` hook subscribes any component to it:

```tsx
import { use } from '@expressive/react';

const player = MediaPlayer.new(); // owned elsewhere

function NowPlaying() {
  const { track, paused } = use(player);
  return <span>{paused ? '⏸' : '▶'} {track}</span>;
}
```

The component subscribes to what it reads, same as always - but lifecycle stays with the owner. Unmounting doesn't destroy the instance.
...
Copy-on-write is the habit worth keeping from hooks. But when a collection is mutated *a lot* - a game board, a live buffer, rows edited in place - declare it with `has()` and mutate away:
...
class Game extends State {
  ...
  play(index: number, mark: string) {
    this.board.set(index, mark); // updates, in place
  }
}
```

A `has()` list tracks per index, so a component reading `board.get(4)` only re-renders when that cell changes; `map()` does the same for entries keyed by name. See [Arrays & Collections](/docs/guides/collections) for the full contract, or [poke the tic-tac-toe example](/examples/featured/tictactoe) to watch it work.

If you just need to signal "this changed" without replacing the value, `this.set('items')` dispatches an update manually.
...
Writes pass straight through the destructured values, so no alias is needed for most code. Reach for `is` in the one case it earns: retaining the root object alongside sibling values from the same snapshot, like `const { is: counter, count } = Counter.use();`.
