...
```tsx
const { name, email, saving, dirty, is } = UserForm.use();
```

Writes change from `setName(x)` to `is.name = x`. Derived values should usually move to getters, not to extra fields that need to be synchronized.
