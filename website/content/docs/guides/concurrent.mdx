...
description: How Expressive stays consistent under React 18+ concurrent rendering, and how to defer updates with pending()
---
...
## Priority: `pending()` for deferred presentation
...
The updates those writes queue are tagged non-urgent. The tag survives batching and the microtask dispatch, and each subscriber replays through the scheduler it subscribed with — React's `startTransition` in the React adapter. The practical payoff is Suspense behaviour: if the new page suspends, React keeps the current screen on-screen instead of flashing a fallback.

It also returns a promise, resolving once every reader has taken the update — for React, once it commits. That is what a progress flag reads:

```ts
this.busy = true;
pending(() => { app.page = 'settings' }).then(() => { this.busy = false });
```

Four rules:

- The callback runs **synchronously**. Writes after an `await` are outside its scope and need their own `pending()` call.
- It covers **Expressive updates**, not everything in the block. A raw React `setState` alongside these writes keeps its own priority, because the transition bracket is applied per subscriber at replay rather than around the callback.
- **Urgent wins.** If an urgent write invalidates a subscriber already queued at transition priority, the urgent priority takes over — a text input never becomes laggy because a background navigation was in flight.
- Normal batching still applies; `pending()` changes priority, not timing.
