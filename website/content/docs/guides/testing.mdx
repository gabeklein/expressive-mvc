...
Module-scope `State.new()` registers the instance into the global root context - that's the singleton pattern, and it's sticky. Without cleanup, a singleton created in one test file (or at module load) leaks into every test that follows. Reset the root between tests:
...
`pop()` removes every State registered in the root context and runs their cleanup callbacks. This library's own test suite does exactly this, globally. If your tests never touch singletons you won't miss it - the first time one does, you will.
