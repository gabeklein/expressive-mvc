...
They also double as a decomposition tool: pull a busy `.map` or chunk of chrome out of `render()` into a named section. [See them in action](/examples/component/subcomponents).
...
Per-feature error handling without nesting `<ErrorBoundary>` wrappers. See the full recovery flow at [component/boundary](/examples/component/boundary).
