...
Destructuring breaks the binding between a local variable and the property, so `count = 5` just reassigns the local. `is` gives you back a reference to the live instance. Destructure it first so the instance handle is easy to spot, and usually alias it to the state concept (`counter`, `form`, `dashboard`) instead of using `is` directly.
