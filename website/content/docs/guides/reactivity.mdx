...
- A reactive computed: a class getter on a State subclass.
...
The component re-renders when any tracked property changes, and _only_ when a tracked property changes. A sibling changing `items` when you only read `total` does not re-render this component unless `total` actually changed value.

<br />

## Computed values

A reactive computed is a property whose value is derived from other properties. It re-runs when any of its tracked inputs change. Use a normal class getter:
...
A getter on a State subclass is automatically reactive. `this` inside the getter is a tracking proxy, so any managed property you read subscribes the computed value to it.
...
- They track dependencies automatically - any property read through `this` creates a subscription. To opt out, read through `this.is` instead.
