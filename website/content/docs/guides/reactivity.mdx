...
> Properties are the unit of tracking, so mutating an array in place (`items.push(...)`) goes unseen - the property was never assigned. See [Arrays & Collections](/docs/guides/collections) for the ways around that, including `has()` for per-index tracking.
...
- [Arrays & Collections](/docs/guides/collections) - mutation, `has()`, `map()`, and per-index tracking.
