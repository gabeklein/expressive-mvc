...
match against the nearest Router rather than an outer Route's base, so a
self-contained flow can navigate without touching the page URL.

Reach for this only when the flow has no address of its own - wizards, modal
flows, screens in a native app. If a view should be linkable or survive a
reload, tabs included, use nested Routes instead.

The component that owns the flow owns its Router, like any other child state:
...
`/review`. The Wizard can call `this.router.goto()` or `back()` directly, and
its location resets whenever it remounts. Inner navigation never touches the
outer Router, and `back()` past the first entry does nothing rather than
escaping to the page.

Nest a headless `Router`, never `BrowserRouter` - two browser bindings would
compete for the same History API.
