...
| `none`           | Matches when no sibling in this scope did. Root = app 404; nested = section 404.                                      |
...
The nested `none` Route catches only misses below `/missions`. A flat
...
| `null`                        | Cede the path to the nearest scoped `none` Route. |
...
scope with a `none` child so there is somewhere local to fall through.
...
Nest Routes when screens share one address space. Nest a Router only when a UI
region needs an independent location and history. A standalone or one-off
memory flow needs no subclass:
...
Subclass Router when the route set or navigation policy is reusable. An
ordinary outer Route decides when that feature is mounted:
...
The browser matches `/settings`. Inside it, `PanelRouter` moves between
`/profile` and `/security` without changing the browser URL. Its Routes are
authored inside `render()`, so the outer lexical matcher does not interpret
them. Descendants use the nearest Router, and Back stops at that Router's oldest
entry instead of escaping into its parent.

Direct Route siblings need no anonymous root. Add a root Route when it provides
shared `as` layout, a local `none`, nested paths, or a NavLinks tree.
...
Normal Component ownership determines persistence. A Route-owned `PanelRouter`
resets when its placement unmounts and remounts. To preserve its location and
history, store `panel = new PanelRouter()` on a longer-lived State or Component
and render that owned instance.

When the entry location varies by placement instead of belonging to the flow,
mount it explicitly so the caller can pass `path`:

```tsx
<Route to="settings"><PanelRouter path="/security" /></Route>
```

Nest `Router`, not `BrowserRouter`: two browser bindings would compete for the
same address and History API.

A top-level Route already creates and owns a fallback headless Router when it
cannot find one upstream. That supports standalone route trees, but it does not
isolate a Route beneath an existing Router. Use a Router subclass when the
feature deliberately needs its own navigation boundary.
