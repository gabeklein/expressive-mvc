...
history, store `panel = new PanelRouter({ path: '/profile' })` on a longer-lived
State or Component and render that owned instance. Nest `Router`, not
`BrowserRouter`: two browser bindings would compete for the same address and
History API.
