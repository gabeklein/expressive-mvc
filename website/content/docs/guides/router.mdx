...
navigation behavior. An ordinary outer Route decides when the feature is
mounted; the Router supplies an independent location and history to its
internal Routes:
...
created with `PanelRouter.new({ path: '/profile' })` survives placement, so it
can retain a flow's location and history. Nest `Router`, not `BrowserRouter`:
two browser bindings would compete for the same address and History API.
