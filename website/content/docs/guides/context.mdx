...
> **Good to know:** <br/>
> Root globals are honest about ambiguity. Create two implicit instances of the same type via `State.new()` and neither is "the" global instance - both are evicted from root lookup, rather than one silently winning. Explicit registration (a Provider, or `Context.root.add(state, true)`) bypasses this.
