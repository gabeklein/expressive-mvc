...
Not everything wants a Provider. Some state is genuinely global - the session, the viewport, app settings. Declare those with `static global` and call `State.new()` at module scope:
...
`State.new()` activates the instance against `Context.root`, the global registry every context ultimately inherits from - and because `Session` declares `static global`, it registers there too. Any component, anywhere, can now call `Session.get()` - no Provider required:
...
The declaration is the opt-in. Without it, a context-less `State.new()` instance is fully functional but private - it can read declared globals, but nothing can find *it*. And `static global` must be declared per class: a subclass of a global that doesn't redeclare it throws on activation, so an extended global can't leak into the shared root by accident.

A full app built this way (viewport, session, and theme as globals) is at [composition/globals](/examples/composition/globals).
...
> **Good to know:** <br/>
> Root globals are honest about ambiguity. Create two implicit instances of the same global class via `State.new()` and neither is "the" global instance - both are evicted from root lookup, rather than one silently winning. Explicit registration (a Provider, or `Context.root.add(state, true)`) bypasses this.
