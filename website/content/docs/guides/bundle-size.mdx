...
| `@expressive/react` — `State` only | ~8.5 kB |
...
The React adapter has a **floor you cannot shake below: ~8.5 kB**, reached by importing anything at all from it. That is deliberate and worth being plain about. The adapter works by patching types it does not own, at import time:
