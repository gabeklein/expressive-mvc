---
title: Comparisons
description: How Expressive MVC relates to hooks, Zustand, Redux, and MobX
---
...
function UserProfileView({ userId }) {
  const { is: profile, name, email, loading, save } = UserProfile.use({ userId });
