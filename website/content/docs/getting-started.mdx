...
The same package covers React Native and Expo. The adapter imports only `react` and `react/jsx-runtime`, so it works on any renderer, and Metro resolves it without extra configuration.
