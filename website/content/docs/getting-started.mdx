...
There is nothing renderer-specific to port — the adapter imports only `react` and `react/jsx-runtime` — and it has been exercised on an iOS simulator: render and re-render on write through the native renderer, context through a `Provider`, suspense, and error recovery. Every release additionally gates on a check that installs the published tarballs into a throwaway Expo app (SDK 57, React Native 0.86) and asserts Metro resolves them for iOS and Android, that the build compiles to Hermes bytecode, and that class fields keep their semantics under either of Metro's transform paths. Support is called **provisional** for one reason: no physical device is tested. Two things to know:
