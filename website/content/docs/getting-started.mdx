...
Support is **provisional**. Every release is gated on a check that installs the published tarballs into a throwaway Expo app (SDK 57, React Native 0.86) and asserts Metro resolves them for iOS and Android, that the build compiles to Hermes bytecode, and that class fields keep their semantics under both of Metro's transform paths. What that check does *not* cover is the renderer itself — no device or simulator run is part of CI, so parity with React DOM follows from the adapter importing nothing platform-specific rather than from a measurement. Two things to know:
