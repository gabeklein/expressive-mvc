...
Runs once after activation, wherever the instance is created - including during server render. Return a cleanup function to run on destruction, or `void`. Setup that must accompany the instance itself goes here; client-only resources (DOM, timers, subscriptions) belong in `mount()`.
...
### `mount()` <small>(React)</small>

```ts
class Ticker extends State {
  elapsed = 0;
...
Runs once when the component that **owns** the instance commits - `State.use()`, a `<Provider>` entry it created, or a Component rendered as JSX. Return a function to run on unmount.

Never runs during server render, and never for an instance merely observed or placed - `.get()` and `{instance}` reach an instance the component does not own, and `State.new()` has no component at all. Client-only effects (DOM, timers, subscriptions) belong here; setup that must accompany the instance itself belongs in `new()`.
