...
These exports are framework-agnostic. They're available from both `@expressive/mvc` and `@expressive/react`.
...
```ts
import { watch } from '@expressive/mvc';
...
```ts
import { listener } from '@expressive/mvc';
...
`@expressive/react` re-exports everything from `@expressive/mvc`:
...
So a React project never needs to import from `@expressive/mvc` directly.
