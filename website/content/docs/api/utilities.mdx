...
This is a headless seam. React components subscribe through [`State.use()` or `State.get()`](/docs/api/hooks), so reaching React with a custom observable means wrapping it in a State; there is no hook that subscribes to an arbitrary object.
...
- [Hooks](/docs/api/hooks) - `State.use` and `State.get`, the React subscription paths.
