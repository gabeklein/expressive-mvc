...
An effect which suspends by throwing a promise holds settlement through its retry and any downstream updates the retry causes. Fulfillment and rejection both retry through MVC dispatch. Cancelling the effect or destroying its state releases the hold and prevents the settled promise from reviving it.
