...
Pass a factory to spawn values. Factory maps are string-keyed: `add(key)` runs the factory for a vacant key - an occupied key throws - while `set(key)` with no value respawns through the factory, replacing the previous value. Keyless `add()` derives the key from `String(value)` - for a `State`, its natural id. Spawned `State` values are owned by the map - destroyed when deleted, cleared, or replaced - while values supplied via `set(key, value)` are guests and never destroyed. A `State` class may stand in for the factory: `add(key?)` instantiates it, forwarding the key to the constructor.
