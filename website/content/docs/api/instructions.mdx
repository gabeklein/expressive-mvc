...
`map()` is a proper instruction: the field resolves into a reactive `State.Map` when the hosting state activates, which adopts the map in the same step. It is not usable standalone.

Pass a factory to spawn values. Factory maps are string-keyed. `add(input?)` always passes `input` to the factory; a string input is also the key - an occupied key throws - while any other input is factory-only, the key deriving from the value: a `Component` supplies its `key`, anything else falls back to `String(value)` (a `State`'s natural id). `set(key)` with no value respawns through the factory, replacing the previous value. Spawned `State` values are owned by the map - destroyed when deleted, cleared, or replaced - while activated values supplied via `set(key, value)` are guests and never destroyed. Every map is adopted by its hosting state when the instruction resolves: fresh members are parented to the owner, activate inside its context, and are destroyed with it - already-activated values keep guest status. The field is read-only, and a `State` value that dies evicts itself from every key it occupies. `keys(fn)`, `values(fn)`, and `entries(fn)` return reusable iterables of transformed results - a fresh pass over live entries per iteration, tracking like their plain forms; `throw false` in the callback skips an entry. A `State` class may stand in for the factory: object input assigns fields via the constructor, while string input keys the entry and arrives as `{ key: input }` - a `Component` adopts it as its identity `key` before the `new()` hook runs; classes with no `key` surface ignore it.
...
class Cart extends State {
  ...
  add(id: string) {
    return this.items.add(id); // spawns Line for vacant id
  }
}
```
