...
Pass a factory to spawn values from keys. `add(key)` is get-or-create: a new key runs the factory, a known key returns the existing value. Spawned `State` values are owned by the map - destroyed when deleted, cleared, or replaced - while values supplied via `set` are guests and never destroyed. A zero-arity factory instead returns a whole `[key, value]` entry, and `add()` takes no arguments. A `State` class may stand in for the factory: `add(input?)` instantiates it, forwarding `input` to `Type.new()`, keyed by the instance's natural id.
