...
// backed by a native Set at runtime (instanceof Set, not Map)
interface map.Set<V, A extends unknown[] = []> {
  ...
  // plus size/clear/values/forEach/iteration over values
}
```
