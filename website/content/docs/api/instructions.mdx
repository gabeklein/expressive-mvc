...
The most versatile instruction. Handles default values, placeholders, lazy/async factories, and validation callbacks.

For reactive computed values, use a normal class getter - getters on a State subclass are auto-promoted to memoized, dependency-tracked properties. See [Reactive computed (getters)](#reactive-computed-getters) below.
