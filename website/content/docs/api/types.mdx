...
Not currently used by any public signature - `set(assign)` takes [`State.Assign<T>`](#stateassignt), which preserves declared field types rather than unwrapping them.
...
Phase timing is covered under [`State.on`](/docs/api/state#stateonhandler).
...
> The `Record<string, unknown>` intersection means unknown keys pass the type-checker (they're ignored at runtime) - value-correctness of declared fields is the only check. See [What `Assign` checks](/docs/api/state#what-assign-checks).
