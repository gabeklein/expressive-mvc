...
**Breaking:** a scope holding a section 404 now shadows later siblings declared under its path, per the usual first-match declaration order. A flat `<Route to="docs/team/roster">` declared after a `docs` section that owns a `default` was reachable before and is not now - move it above the section to restore it.
