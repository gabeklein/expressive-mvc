 ...
 *
 * @param fn - Factory function to generate a getter to subscribe dependancies.
 */
export function from <R> (fn: (property: string) => (this: any, state: any) => R): R;
 ...
 * 
 * @param fn - Compute function. Bound to a subscriber-proxy of source, returns output value.
 * Will update automatically as input values change.
 */
export function from <T extends Class, R> (source: T, fn: (this: InstanceOf<T>, on: InstanceOf<T>) => R): R;
 ...
 * Will update automatically as input values change.
 */
export function from <T, R> (source: T, fn: (this: T, on: T) => R): R;
...
export function pending <T> (waitFor: () => Promise<T>): T;

/**
 * Suspend for a computed value; output will be generated by provided function.
 * Will throw a promise if value is accessed while undefined, promise will
 * resolve once value gains any value.
 *
 * @param source - Source model from which computed value will be a subscriber.
 * 
 * @param fn - Compute function. Bound to a subscriber-proxy of source, returns output value.
 * Will update automatically as input values change.
 */
 export function pending <T, R> (source: T, fn: (this: T, on: T) => R): Exclude<R, undefined>;