...
declare class FancyMap<K, T> extends Map<K, T> {
  get(key: K): T | undefined;
  get(key: K, callback: (value: T) => void): () => void;
}

declare function map<T>(): FancyMap<string, T>
declare function map<K = string, T = unknown>(): FancyMap<K, T>
  ...
  static createRef(property: string): Ref<HTMLInputElement> {
    return this.get(self => {
      ...
      return (input) => {
        ...
        const unfollow = values.get(property, x => input.value = x);
        const onInput = () => values.set(property, input.value);
      
        if(values.has(property))
          input.value = values.get(property)!;
        else
          onInput();
