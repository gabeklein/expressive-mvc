...
`has.List` and `has.Pool` are the runtime classes (`has.Create` is the class-mode pool's type) - mode is class identity (`instanceof` works; a list has no `add`, a pool no `push`, as natural TypeErrors). Adapters may extend their prototypes - this is the seam for rendering facades.
