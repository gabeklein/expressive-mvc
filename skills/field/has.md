...
`has.List` and `has.Pool` are the runtime classes (`has.Create` and `has.From` type the class-mode pool) - mode is class identity (`instanceof` works; a list has no `add`, a pool no `push`, as natural TypeErrors). Adapters may extend their prototypes - this is the seam for rendering facades.
