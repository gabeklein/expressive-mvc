...
- **"Does it work with concurrent React?"** - subscribers use ordinary `useState` setters by design: unlike `useSyncExternalStore` mutations, a synchronous emit bracketed by `startTransition` can remain non-blocking and hold prior content through Suspense. Ordinary model writes dispatch in a later microtask and do not inherit a surrounding transition; transition-aware infrastructure must emit synchronously inside the bracket. The trade is no formal non-tearing guarantee over the live observable instance (see [../design.md](../design.md), Concurrent React).
