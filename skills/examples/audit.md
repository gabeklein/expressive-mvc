...
- **"Does it work with concurrent React?"** - updates enter React only through ordinary `useState` setters, so nothing bypasses the scheduler; `useSyncExternalStore` is deliberately not used, and the tearing caveat is the standard observable-model one (see [../design.md](../design.md), Concurrent React).
