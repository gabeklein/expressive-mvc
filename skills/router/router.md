...
A see-through scope counts as matched when a descendant matches, or when it owns a `default` - which claims anything under the scope's path, so the section 404 answers there instead of the app one. Without a default the scope is never a greedy prefix. That verdict is the same one sibling `as`-routes arbitrate over, so a scope holding a section 404 also shadows later siblings under its path. Put routes where they are lexically visible to the matcher.
