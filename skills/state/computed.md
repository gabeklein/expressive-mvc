...
A getter declared on a State subclass is auto-promoted to a memoized, dependency-tracked property. No instruction is needed - just write a normal class getter.

## How It Works

When State bootstraps a class, it walks the prototype and finds every class-syntax getter (descriptors with `configurable: true` and no paired setter). Each one is installed on the instance as a managed property:
