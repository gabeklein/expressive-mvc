...
The adapter is renderer-agnostic, so the same install covers React Native and Expo - exercised on an iOS simulator in both Debug and Release builds (Expo SDK 57, React Native 0.86) and gated per release on Metro resolution and a Hermes build. See [React Native](skills/react/react.md#react-native) for the two boundaries.
