...
With Expressive, you're able to create and use simple classes as controllers, via built-in hooks, within components. 

When built-in methods on a **Model** are used within a component **(View)**, an instance is either found or created for its use. By watching what's used on first render, the hook itself **(Controller)** can keep up to date with those values.
...
First, create a class and extend `Model` shaping it to your liking.
...
Next pick a built-in hook, such as `.use()`, to make for a stateful component.
...
Ultimately the workflow is nice and simple. If you know [MobX](https://mobx.js.org/README.html) this will look pretty familiar, but also *a lot* more straight-forward.
...
3. Within a component, call one of the built-in methods, as any normal custom hook.
4. Destructure out values (in the component ofcourse) to make use of and subscribe.
...
For that, here's some library jargon which will be good to know.
 ...
 - **State**: The stateful proxy, specific to a component, managing its behavior. 
 - **Subscriber**: State's better half, an open channel to MVC engine, responding to events.
...
Alrighty, let's get started :clap:

<br/>

<h2 id="concept-simple">Simplest use-case</h2>

Let's back up from the first example, and start with a bare minimum.
...
One of the static-methods added to your class will be `use`. This is a hook; it will create a new instance of your state and bind it to the component.

Now, we can just change values as we see fit! As values on this instance change, our hook handles any need for new renders.

<br/>

<h2 id="concept-destruct">Destructuring</h2>

Subscribers are lazy, they will only update a view for values they think are in-use. See [that section](#concept-subscription) for more detail.

A good idea is to destructure values needed for a component, that way conditional rendering doesn't lead to some unexpected behavior.

Now, you might ask how we still access our instance, having already destructured. To cover for this, you'll see a **`set`** and **`get`** included with the returned state.

> Not to be confused with the keywords! They're just properties, and both a simple reference back to state.

### `set`

Having destructured, let's use `set` for updating values on our instance.
...
Exactly the same as `set` but for other purposes while also keeping things readible. Most of the time, you'll use `get` to pull information without also implying you want updates. <br/>

Usually, when you read an observable value directly, a controller assumes you want to refresh anytime that property changes. In plenty of situations, this isn't the case, so `get` serves as an escape-hatch.

<br/>

```js
const { get: instance, value } = MyModel.use();
```
...
Also worth mentioning, you can use it for bracket-notation too (i.e. `get["property"]`), and generally to avoid clutter. 

<br/>

<h2 id="concept-method">Adding methods</h2>

What's a class without some methods? Let's add some *actions* [(similar to that of MobX)](https://mobx.js.org/refguide/action.html) to easily abstract out changes to our state.

```jsx
class CountControl extends Model {
  number = 1

  // Note that we're using arrow functions.
  // In-practice, this works a lot like `useCallback`!
...
> You may notice this approach is cleaner, but also more efficient than inline-handlers. These callbacks won't make a new closure every time we render now. 😬
...
You'll be happy to know Expressive has a strong equivalent to "computed" properties [(ala MobX)](https://mobx.js.org/refguide/computed-decorator.html).

All you need to do is define a `get()` method; it will be automatically managed by the controller. Computed values are run only when first accessed, and actively kept in-sync thereafter.

Through the same [mechanism]() as hooks, getters know when the specific properties they've accessed are updated. Whenever that happens, they rerun. If a new value is returned, it will be passed forward to the getter's own listeners.
...
> **Important Caveat:** Controller getters are cached, facing the user. They will only run when a dependency changes, and **not** upon access (besides initially) as you might expect.
...
The method `use(...)`, as it creates the control instance, will pass arguments to your model's constructor. This makes it easy to customize state for a component.
...
<h2 id="concept-passing-props">Pass props to your controller instead</h2>
...
```
> Here we expect Greetings to "export" **firstName** & **isBirthday**. To do this effectively, it will need first to "import" **name** and **birthday**, which start out undefined. We pass in `props` to facilitate this.
...
```
> Just make sure to define in those props. Pretty easy!
```jsx
const SayHello = () => (
  <HappyBirthday
    name="John Doe"
...
### ✅ Level 1 Clear!
> In this chapter we learned the basics of how to create and utilize a custom state. For most people, who simply want smart components, this could be enough! However, we can make much more than just a fancy hook.

<br/>

<h1 id="managing-section">Managing a dynamic state</h1>

So far, all of our example controllers have been passive. Next steps will beto give our controller a bigger roll, pushing updates without direct user interaction.

Because state is ultimately just an object, we can do whatever we want to values, and more-crucially, whenever and wherever. This makes asynchronous coding pretty low maintenance. We handle the logic for what we want and the controller will handle the rest.
...
Built-in hooks will automatically call a number of "special methods" you'll define on your model, to handle certain "special events" within components.
...
Beyond watching for state-change, what a subscriber really cares about is events. *Updates are just one cause for an event.* Whenever a property on your state gains a new value, subscribers are notified and then act accordingly.

While usually it'll be a controller waiting to refresh a component, anything can subscribe to an event via callbacks. If this event *is* caused by a property update, its new value will serve as an `argument`; if synthetic however, that will be up to the dispatcher.
<br /><br />
...
Instances of `Model` have the following methods added in for event handling:
...
The method also returns a callback, with which you can stop subscribing. 

> **Protip:** You do not need to cleanup events yourself, before expiring state. Built-in events such as `willDestroy` and `componentWillUnmount`, will  stop added listeners for you.

#### `state.once(name, callback) => onCancel`

Same as `on`, however will delete itself after being invoked. You can still cancel it though with the returned callback.
...
#### `state.watch(arrayOfNames, callback, once?) => onDone`
...
- Property exists:
  - **no argument:** All subscribers will force update, listeners will get the existing value.
...
- property is a getter:
  ...
  - **has argument:** Cache will be overwritten (compute skipped), and listeners get that value.
...
This method helps integrate outside values by assigning argument-properties **every render**. Because the observer will only react to *new* values, this makes for a fairly clean way to watch props. We can combine this with getters and event-listeners, to do all sorts of things when inputs change.
...
```
> Like this, we can freely interact with different sources of state!
...
>
> If we want to modify or even duplicate our `ActionSequence`, says in a different language or with a new aesthetic, we don't need to copy or edit any of these actual behaviors. 🤯

However we can take this a step forwards, becasue with classes, we can _extend_ logic just as easily as reuse it.

<br/>

<h1 id="extending-section">Extending state</h1>
...
By default, a `Model` is biased towards context as it's sharing mechanism. You probably guessed this, but through a managed [React Context](https://frontarm.com/james-k-nelson/usecontext-react-hook/) can we create and consume a single state throughout a component hierarchy.
...
```ts
export class Central extends Model {
  foo = 0;
  bar = 0;
...
<h2 id="managing-section">Global Models</h2>
...
Here we introduce a new class of controller called a `Singleton` (or `GC` for short). With it we can create shared state without caring about hierarchy! Access hooks work exactly the same as their `Model` counterparts, except under the hood they use a single promoted instance.
