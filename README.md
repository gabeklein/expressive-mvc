...
The adapter is renderer-agnostic, so the same install covers React Native and Expo - exercised on an iOS simulator (Expo SDK 57, React Native 0.86) and gated per release on Metro resolution and a Hermes build, provisional only in that no physical device is tested. See [React Native](skills/react/react.md#react-native) for the two boundaries.
