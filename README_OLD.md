...
<p align="center">
  ...
  <!-- <a href=""><img alt="Build" src="https://shields-staging.herokuapp.com/npm/types/@expressive/state.svg"></a> -->
...
Classes which extend `State` can manage behavior for components. States are easy to extend, use and even provide. While hooks are great, state and logic are not a part of well-formed JSX. States help wrap that stuff in robust, typescript controllers instead.
...
> States use first-render access to decide what warrents an update when something changes. Naturally, properties _you do not access_, will not cause an update. For this reason, it is the recommended pattern to always destructure.

<br/>

### Refresh using simple assignment

Reserved `is` loops back to the instance, to update from inside a component.
...
const MyComponent = () => {
  const { name, is } = Hello.use();

  return <p onClick={() => (is.name = 'Foobar')}>Hello {name}!</p>;
};
```
...
```jsx
import State, { Provider } from '@expressive/state';
...
const Child = () => {
  ...
  return <p>Shared value is: {value}!</p>;
};
```
...
```ts
abstract class About extends State {
...
<!-- **Customizing** <br/>
  &ensp; •&nbsp; [Extending Custom States](#concept-extension) <br/>
...
```bash
npm install --save @expressive/state
```
...
- **State**: Any class you'll write extending `State`. Defines a type of state.
- **Controller**: An instance of State, containing its state and behavior.
...
[State hooks are lazy](#concept-lazy), and will only update for values seen to be in-use.
...
```js
const { get: state, value } = State.use();
```
...
Getters run whenever a controller thinks they _could_ change. Make sure to design them along these lines:
...
<h4 id="method-uses"><code>State.uses(source, keys?)</code></h4>
...
class Greetings extends State {
  ...
  get isBirthday() {
    ...
    return td.getMonth() === bd.getMonth() && td.getDate() === bd.getDate();
  }
}
```

Greetings defines _firstName_ & _isBirthday_; however they themselves depend on _name_ and _birthday_, which start out undefined. We can pass in `props` to help with that.

```jsx
const HappyBirthday = (props) => {
  const { firstName, isBirthday } = Greetings.use(props);
...
```jsx
const SayHello = () => <HappyBirthday name="John Doe" birthday="January 1" />;
```
...
```js
const state = State.uses({ ... }, ["name", "birthday"]);
```
...
So far, all our examples have been passive. States can serve a bigger roll, however, to evolve state even without user input.
...
```jsx
class TimerControl extends State {
  interval = undefined;
  elapsed = 1;
...
Besides state change, what a subscriber really cares about is events. _Updates are just one source for an event._ When any property gains a new value, subscribers are simply notified and act accordingly.
...
> By default, callbacks are run once for every key/value update, even if multiple happen at the same time. If you'd rather know when an event happens for _any_ key, squash will fire once for a frame, with a list of keys updated.
...
```js
class TickTockClock extends State {
  constructor() {
    super();
  ...
  tickTock(seconds) {
    if (seconds % 2 == 1) console.log('tick');
    else console.log('tock');
  }

  logMinutes(minutes) {
    if (minutes) console.log(`${minutes} minutes have gone by!`);
  }
...
<h4 id="method-using"><code>State.using(source, keys?)</code></h4>

> Roughly equivalent to [`uses()`](#concept-passing-props)

This method helps watch an object by running [`assign`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/assign) on it _every render_. Because controllers only react to new values, this makes for a simple way to watch externals. Combine this with getters and event-listeners, to do things when inputs change.

```ts
class ActivityTracker extends State {
  active = undefined;
  ...
  constructor() {
    ...
    this.on('active', (yes) => {
      if (yes) alert('Tracker just became active!');
    });
  }
}
```
...
const DetectActivity = (props) => {
  ...
  return <big>This element is currently {status}.</big>;
};
```
...
```ts
class StickySituation extends State {
  agent = 'Bond';
  remaining = 60;
  ...
  tickTock = () => {
    ...
    if (timeLeft === 0) this.update('oh no!');
  };
...
const ActionSequence = () => {
  const { agent, getSomebodyElse, remaining } = StickySituation.use();

  if (remaining === 0) return <h1>🙀💥</h1>;
...
>
> However we can keep going, because with States, _sharing_ logic means more than just reusing logic.

<br/>
<h1 id="sharing-section">Sharing state</h1>

One of the most important features of a State is the ability to share a single, active state with any number of users, be them components or even [other controllers](#concept-ambient). Whether you need state in-context or to be usable app-wide, you can, with a number of simple abstractions.
...
Start with a normal, run-of-the-mill State.

```js
export class FooBar extends State {
  foo = 0;
  bar = 0;
}
```
...
```jsx
import { Provider } from '@expressive/state';
...
Where `FooBar.use()` creates an instance of `FooBar`, `FooBar.get()` fetches an _existing_ instance of `FooBar`.
...
> There's another big benefit here: _types are preserved._
>
> States, when properly documented, maintain autocomplete, hover docs, and intellisense, even as you pass controllers all-throughout your app! 🎉
...
By default, `State` uses [React Context](https://frontarm.com/james-k-nelson/usecontext-react-hook/) to find instances from upstream.
...
>
> States and their consumers do not need to live in same file, let alone module. Want to interact with another library's state? This can make for a pro-consumer solution. ☝️😑
...
Without constructor-arguments, creating the instance separately can be an unnecessary step. Pass a State itself, to both create and provide a state in one sitting.
...
When a State is passed to `of` directly, the Provider will behave similarly to [`State.using()`](#concept-external). All other props are forwarded to state and likewise observed.
...
const MockFooBar = () => {
  ...
  return <Provider of={{ hello, Foo, Bar }}>{props.children}</Provider>;
};
```

<br/>

<h1 id="section-global">Global States</h1>
...
For this reason we have `Singleton`, a type of state to exist everywhere. Hooks will present the same as their `State` counterparts, but under the hood, always retrieve a single, promoted instance.

<br/>

## Defining a Singleton

Simply extend `Singleton`, itself a type of `State`.

```js
import { Singleton } from '@expressive/state';
...
A method on all States, `create` on a Singleton will also promote that new instance. This can be done anytime, as long as it's before a dependant (component or peer) tries to pull data from it.
...
Whether a state extends `State` or `Singleton` won't matter; they both present the same.<br/>
...
States make fetching a state super easy. On every model, there are defined three `useContext` like methods. They find, return and maintain the nearest instance of class they belong to.
...
Hooks are great, but not all components are functions. For those we have the `Consumer` component. This is also generic, and able to capture state for any State passed to its `of` prop.
...
On your State, assign a property with the value of `tap(State)`, passing in the type of state you're interested in.
...
```js
import State, { tap } from '@expressive/state';

class Hello extends State {
  to = 'World';
}

class Greet extends State {
  hello = tap(Hello);
...
Here _SayHi_, without direct access to the _Hello_ controller, can still get what it needs. The actionable value `greeting` comes from _Greet_, which itself gets data from _Hello_. This is an example of scope-control, allowing code to written as needs-based. Generally speaking, this often makes a program more robust and maintainable.
...
<br/>
<h1 id="composing-section">Composing States</h1>

Ultimately the purpose of States, and be that classes in-general, is to compartmentalize logic. This makes code clear and easy to duplicate. Ideally, we want controllers to be really good at one thing, and to cooperate with other systems as complexity grows.
...
Import another instruction, `use` and pass it a State to attach a child-instance upon creation.

```js
import State, { use } from '@expressive/state';
...
While separating concerns, there will be times where designing a State expressly to help another makes sense, and a reference to that parent is desirable. For this we have the `parent` instruction.
...
```js
import State, { use, parent } from '@expressive/state';

class Control extends State {
  child = use(Dependant);
}

class Dependant extends State {
...
> Here we've learned how States allow controllers to cooperate and play off of eachother. They allow us to break-up behavior into smaller pieces, building-blocks if you will. With that, we have an easy way to separate concerns, freeing up focus for more added nuance and features.
...
Good things to keep in mind of as you build with State patters in your apps. Naturally, just suggestions, but these are design principles to be supported as this library evolves.

<br/>
<h2 id="concept-typescript">States & Typescript</h2>

Typescript is your friend and Expressive is built laregely to facilitate its use within React apps.

A ton of focus has been put in to make sure _all_ features of Expressive are in-line with what Typescript lanaguage supports. Transparency is key for a great developer experience, and State's are designed to infer often and fail-fast in static analysis.

```ts
import State from '@expressive/state';

class FunActivity extends State {
...
const PaintDrying = ({ alreadyMinutes }) => {
  ...
  return (
    <div>
      I've been staring for like, {secondsSofar} seconds now, and I'm starting
...
<h1>Concepts</h1>
Topics to help in understanding how State's work under the hood.

<br/>
<br/>
<h2 id="concept-lazy">Subscription based "lazy" updating</h2>

Controllers use a subscription model to decide when to render, and will **only** refresh for values which are actually used. They do this by watching property access _on the first render_, within a component they hook up to.

That said, while hooks can't actually read your function-component, destructuring is a good way to get consistent behavior. Where a property _is not_ accessed on initial render render (inside a conditional or ternary), it could fail to update as expected.
...
```jsx
class ZeroStakesGame extends State {
  foo = 'bar';
  bar = 'baz';
  baz = 'foo';
...
<h2 id="controller-api">State</h2>

Set behavior for certain properties on classes extending `State`.
