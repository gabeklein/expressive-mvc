...
Classes which extend `Model` can manage behavior for components. Models are easy to extend, use and even provide. While hooks are great, state and logic are not a part of well-formed JSX. Models help wrap that stuff in robust, typescript controllers instead.
...
```
>  Models use first-render access to decide what warrents an update when something changes. Naturally, properties _you do not access_, will not cause an update. For this reason, it is the recommended pattern to always destructure.
