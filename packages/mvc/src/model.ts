...
const PENDING_EVENT = new WeakMap<Model, Set<Event>>();
...
function emit(source: Model, key: Event){
  ...
  let pending = PENDING_EVENT.get(source);
  ...
  PENDING_EVENT.set(source, pending = new Set(isActive ? [key] : [true, key]));
