...
namespace Observable {
  export type Callback<T> = (object: T, key: unknown, value: unknown) => void;
}

interface Observable {
  [OBSERVE](callback: Observable.Callback<this>): this;
}
...
function createEffect<T extends Model>(target: T, callback: Effect<T>, argument?: boolean){
  ...
  function invoke(){
    ...
    const subscriber = target[OBSERVE](access);
    ...
    function access(from: Observable, key: unknown, value: any) {
      ...
      listener.add(key as string);

      const nested = LISTENERS.get(value);

      if(nested)
        LISTENERS.set(value = value[OBSERVE](access), nested);
