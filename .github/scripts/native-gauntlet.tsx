...
function Renderer({ report }: { report: Report }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      report('write re-renders through the native renderer',
...
export default function App() {
  const [log, setLog] = useState<[string, boolean, string][]>([]);

  const report: Report = (name, pass, detail) =>
    setLog(prev => [...prev, [name, pass, detail]]);
  ...
  const done = log.some(([name]) => name === 'GAUNTLET');

  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <Text testID="summary" style={styles.summary}>
        {done ? failed ? `FAIL ${failed}` : `PASS ${log.length - 1}` : 'running'}
      </Text>
